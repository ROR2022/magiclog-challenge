...
const Register = () => {
  ...
  const [roleValue, setRoleValue] = useState(new Set(["buyer"]));
  ...
  const handleRegisterUser = async (values: any) => {
    try {
      ...
      const userRoles = Array.from(roleValue);

      if (imageUser) {
        formData = new FormData();
        formData.append("file", imageUser);
        formData.append("status", "active");
        formData.append("roles[]", userRoles[0]);
        for (let key in values) {
          formData.append(key, values[key]);
        }
      } else {
        values = { ...values, roles: userRoles };
        formData = values;
      }
  ...
  const handleSelect = (e: any) => {
    ...
    setRoleValue(new Set([currentKey]));
  };
